...
class EmbeddedController(cmd.Cmd):
    ...
    def __init__(self):
        ...
        self.yaw   = rospy.Publisher(qubo_namespace + 'yaw_cmd', Float32,
                                     queue_size=5)
        self.pitch = rospy.Publisher(qubo_namespace + 'pitch_cmd', Float32,
                                     queue_size=5)
        self.roll  = rospy.Publisher(qubo_namespace + 'roll_cmd', Float32,
                                     queue_size=5)
        self.depth = rospy.Publisher(qubo_namespace + 'depth_cmd', Float32,
                                     queue_size=5)
        self.surge = rospy.Publisher(qubo_namespace + 'surge_cmd', Float32,
                                     queue_size=5)
        self.sway  = rospy.Publisher(qubo_namespace + 'sway_cmd', Float32,
                                     queue_size=5)
