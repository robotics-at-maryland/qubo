...
from utils.utils import logging, warning, file_lines
...
valid_interval = train_options['valid_interval']
dot_interval  = 70  # batches

# Test parameters
mask_threshold   = thres['mask_threshold']
...
test_loader = torch.utils.data.DataLoader(
    dataset.ListDataset(testlist, shape=(init_width, init_height),
                   ...
                   transform=transforms.Compose([
                       transforms.ToTensor(),
                       transforms.Normalize(torch.zeros(3),torch.ones(3))
                   ]), train=True),
...
def train(epoch,writer,logger):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        if batch_idx%summary_interval == 0 and writer is not None:
            #print( label.size() )
            #print( label.cpu().data[0:4].shape)
            writer.add_scalar( 'loss', loss.data[0], global_step )
            writer.add_image( 'Image', make_grid(data.cpu().data[0:4], 2), global_step )
            writer.add_image( 'Predict', make_grid(output.cpu().data[0:4], 2), global_step )
            writer.add_image( 'Label', make_grid(label.cpu().data[0:4], 2), global_step )
...
def test(epoch):
    ...
    for batch_idx, (data, label) in tqdm(enumerate(test_loader), total=n_sample):
        if use_cuda:
            data = data.cuda()
            label = label.cuda()
        ...
        output = model(data)
        loss = model.loss(output, label)
        dice_coef.append( loss.cpu().data[0] )

        if batch_idx%summary_interval == 0 and writer is not None:
            writer.add_scalar( 'val_loss', loss.data[0], global_step )
            writer.add_image( 'val_Image', make_grid(data.cpu().data[0:4], 2), global_step )
            writer.add_image( 'val_Predict', make_grid(output.cpu().data[0:4], 2), global_step )
            writer.add_image( 'val_Label', make_grid(label.cpu().data[0:4], 2), global_step )



    logging("dice coef: %f"% (-np.mean(dice_coef)), 'blue')
